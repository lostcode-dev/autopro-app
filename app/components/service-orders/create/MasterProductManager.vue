    ...
    <template #body>
      <div class="space-y-4">
        ...
        >
          ...
          <p class="mt-3 text-sm font-medium text-highlighted">Nenhum produto master encontrado</p>
        </div>
          ...
          >
            <div class="flex items-start justify-between gap-4">
              <div class="min-w-0 flex-1">
                <p class="truncate font-semibold text-highlighted">{{ product.name }}</p>
                <p v-if="product.description" class="mt-1 text-sm text-muted">{{ product.description }}</p>
                <p v-if="product.notes" class="mt-2 text-xs text-dimmed">{{ product.notes }}</p>
              </div>

...
import { formatCurrency, formatDate, formatPhone } from '~/utils/service-orders'
...
const totalAmount = computed(() => Number(detail.value?.order.total_amount ?? 0))
const discountAmount = computed(() => Number(detail.value?.order.discount ?? 0))
...
async function loadData() {
  ...
  try {
    const [detailResponse, organizationResponse] = await Promise.all([
      $fetch<{ data: ServiceOrderDetailFull }>(`/api/service-orders/${props.orderId}`),
...
function getClientPhone() {
  return detail.value?.client?.mobile_phone || detail.value?.client?.phone || null
}
  ...
  >
    ...
    <template #body>
      <div v-if="isLoading" class="space-y-4 p-6 lg:p-8">
        <USkeleton class="mx-auto h-[1120px] w-full max-w-[860px] rounded-[32px]" />
      </div>
      ...
      <div v-else-if="detail" class="px-4 py-6 lg:px-8 lg:py-8">
        <div class="mx-auto max-w-[900px]">
          ...
          >
            <div class="border-b border-slate-200 bg-[linear-gradient(135deg,#f8fafc_0%,#eef2ff_100%)] px-8 py-7 lg:px-10">
              <div class="flex items-center gap-4">
                <div class="flex size-16 items-center justify-center overflow-hidden rounded-2xl bg-slate-950 text-white shadow-lg">
                  <img
            ...
            <div class="space-y-8 px-8 py-8 lg:px-10 lg:py-10">
                ...
                <section class="grid gap-6 sm:grid-cols-2">
                  <div>
                    ...
                    <div class="mt-3 space-y-2 text-sm text-slate-700">
                      <p><span class="font-semibold text-slate-950">OS Nº:</span> {{ detail.order.number ?? '—' }}</p>
                      <p><span class="font-semibold text-slate-950">Data:</span> {{ formatDate(detail.order.entry_date) }}</p>
                      <p><span class="font-semibold text-slate-950">Previsão:</span> {{ formatDate(detail.order.expected_date) }}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div class="mt-3 space-y-2 text-sm text-slate-700">
                      <p><span class="font-semibold text-slate-950">Nome:</span> {{ detail.client?.name ?? '—' }}</p>
                      <p><span class="font-semibold text-slate-950">Telefone:</span> {{ formatPhone(getClientPhone()) }}</p>
                      <p v-if="detail.client?.email">
                        <span class="font-semibold text-slate-950">Email:</span> {{ detail.client.email }}
                      </p>
                    </div>
                  </div>
                </section>
                ...
                <section class="rounded-[24px] border border-slate-200 bg-slate-50/80 p-5 text-right">
                  ...
                  <div class="mt-3 space-y-2 text-sm text-slate-700">
                    ...
                    <p v-if="organization?.phone">
                      <span class="font-semibold text-slate-950">Telefone:</span> {{ formatPhone(organization.phone) }}
                    </p>
                    <p v-if="organization?.mobile_phone">
                      <span class="font-semibold text-slate-950">WhatsApp:</span> {{ formatPhone(organization.mobile_phone) }}
                    </p>
                    <p v-if="organization?.email">
                      <span class="font-semibold text-slate-950">Email:</span> {{ organization.email }}
                    </p>
                    <p v-if="organization?.website">
                      <span class="font-semibold text-slate-950">Site:</span> {{ organization.website }}
                    </p>
                    <div v-if="organization?.address_street || organization?.address_city" class="pt-1 leading-relaxed">
              ...
              <section v-if="detail.vehicle" class="space-y-3">
                ...
                <div class="grid gap-3 rounded-[24px] border border-slate-200 bg-white sm:grid-cols-2 lg:grid-cols-4 p-5">
                  <p class="text-sm text-slate-700">
                    <span class="font-semibold text-slate-950">Marca/Modelo:</span> {{ detail.vehicle.brand }} {{ detail.vehicle.model }}
                  </p>
                  <p class="text-sm text-slate-700">
                    <span class="font-semibold text-slate-950">Placa:</span> {{ detail.vehicle.license_plate ?? '—' }}
                  </p>
                  <p class="text-sm text-slate-700">
                    <span class="font-semibold text-slate-950">Ano:</span> {{ detail.vehicle.year ?? '—' }}
                  </p>
                  <p class="text-sm text-slate-700">
                    <span class="font-semibold text-slate-950">Combustível:</span> {{ detail.vehicle.fuel_type ?? '—' }}
                  </p>
                </div>
              </section>

              <section v-if="detail.masterProduct" class="rounded-[24px] border border-violet-200 bg-violet-50/70 p-5">
                <h3 class="text-sm font-bold uppercase tracking-[0.2em] text-violet-700">
                  Produto Master
                </h3>
                <div class="mt-3 space-y-2 text-sm text-slate-700">
                  <p><span class="font-semibold text-slate-950">Nome:</span> {{ detail.masterProduct.name }}</p>
                  <p v-if="detail.masterProduct.description">
                    <span class="font-semibold text-slate-950">Descrição:</span> {{ detail.masterProduct.description }}
                  </p>
                  <p v-if="detail.masterProduct.notes">
                    <span class="font-semibold text-slate-950">Observações:</span> {{ detail.masterProduct.notes }}
                  </p>
                </div>
              </section>
                ...
                <div class="overflow-hidden rounded-[24px] border border-slate-200">
                  <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                      <tr>
                        <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
                          Descrição
                        </th>
                        <th class="px-4 py-3 text-center text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
                          Qtd
                        </th>
                        <th class="px-4 py-3 text-center text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
                          Valor Unit.
                        </th>
                        <th class="px-4 py-3 text-right text-xs font-bold uppercase tracking-[0.18em] text-slate-500">
                          Total
                        </th>
                      </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200 bg-white">
                      <tr v-for="(item, index) in detail.order.items ?? []" :key="`${index}-${item.name ?? item.description ?? 'item'}`">
                        ...
                        <td class="px-4 py-3 text-right text-sm font-semibold text-slate-950">
              ...
              <div class="flex justify-end">
                <div class="w-full max-w-[320px] rounded-[24px] border border-slate-200 bg-slate-50/80 p-5">
                  ...
                  <div v-if="discountAmount > 0" class="flex items-center justify-between py-2 text-sm text-rose-600">
                    <span>Desconto</span>
                    <span class="font-semibold">- {{ formatCurrency(discountAmount) }}</span>
                  </div>
                  <div class="mt-2 flex items-center justify-between border-t-2 border-slate-200 pt-4 text-base font-bold text-slate-950">
                    <span>TOTAL</span>

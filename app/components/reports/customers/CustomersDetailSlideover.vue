  ...
  >
    <template #header>
      ...
      <div v-else-if="data" class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-base font-bold text-highlighted leading-tight">{{ data.client?.name ?? '—' }}</h2>
      ...
      <div v-else-if="data" class="space-y-4 p-4">
        ...
        <UCard :ui="{ body: 'p-4' }">
          <div class="flex items-center gap-2 mb-3">
            ...
            <p class="text-sm font-semibold text-highlighted">Informações de contato</p>
          </div>
          <div class="space-y-3">
            <div v-if="data.client?.phone" class="flex items-center gap-3 text-sm">
              ...
              <div>
                <p class="text-xs text-muted leading-none mb-0.5">Telefone</p>
                <p class="font-medium text-highlighted">{{ data.client.phone }}</p>
              </div>
            </div>
            <div v-if="data.client?.email" class="flex items-center gap-3 text-sm">
              ...
              <div>
                <p class="text-xs text-muted leading-none mb-0.5">E-mail</p>
                <p class="font-medium text-highlighted">{{ data.client.email }}</p>
              </div>
            </div>
            <div v-if="data.client?.document" class="flex items-center gap-3 text-sm">
              ...
              <div>
                <p class="text-xs text-muted leading-none mb-0.5">Documento</p>
                <p class="font-medium text-highlighted">{{ data.client.document }}</p>
              </div>
            </div>
            <p
        ...
        <div class="grid grid-cols-3 gap-3">
          <div class="rounded-xl border border-default bg-gradient-to-b from-elevated/50 to-default p-3 text-center">
            ...
            <p class="text-sm font-bold text-highlighted leading-tight">{{ formatCurrency(data.stats.totalSpent) }}</p>
            <p class="text-xs text-muted mt-0.5">Total gasto</p>
          </div>
          <div class="rounded-xl border border-default bg-gradient-to-b from-elevated/50 to-default p-3 text-center">
            ...
            <p class="text-sm font-bold text-highlighted leading-tight">{{ data.stats.totalOrders }}</p>
            <p class="text-xs text-muted mt-0.5">Ordens</p>
          </div>
          <div class="rounded-xl border border-default bg-gradient-to-b from-elevated/50 to-default p-3 text-center">
            ...
            <p class="text-sm font-bold text-highlighted leading-tight">{{ formatCurrency(data.stats.averageTicket) }}</p>
            <p class="text-xs text-muted mt-0.5">Ticket médio</p>
          </div>
        </div>
        ...
        <UCard v-if="data.orders.length" :ui="{ body: 'p-0' }">
          <div class="flex items-center gap-2 border-b border-default px-4 py-3">
            ...
            <p class="text-sm font-semibold text-highlighted">Histórico de OS</p>
            <UBadge
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                  <p class="font-semibold text-highlighted leading-tight">OS #{{ order.number }}</p>
                  <UBadge

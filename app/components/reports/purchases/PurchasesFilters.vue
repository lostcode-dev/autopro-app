...
const paymentStatusOptions: TagFilterOption[] = [
  ...
  { value: 'overdue', label: 'Vencido', color: 'error', icon: 'i-lucide-alert-triangle' }
]

const sortedSuppliers = computed(() =>
  [...props.suppliers].sort((a, b) => a.name.localeCompare(b.name, 'pt-BR', { sensitivity: 'base' }))
)
</script>
    ...
    <div class="space-y-3">
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div>
          ...
          <UiTagFilter
            v-model="supplierIds"
            :options="sortedSuppliers.map(supplier => ({ value: supplier.id, label: supplier.name, color: 'neutral' as const, initials: getInitials(supplier.name) }))"

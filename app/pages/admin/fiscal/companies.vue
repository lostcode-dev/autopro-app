...
async function submitForm() {
  ...
  if (!form.value.business_id?.toString().trim() && !form.value.individual_id?.toString().trim()) {
  ...
  try {
    ...
    if (form.value.business_id) payload.business_id = String(form.value.business_id).replace(/\D/g, '')
    if (form.value.individual_id) payload.individual_id = String(form.value.individual_id).replace(/\D/g, '')
    if (form.value.state_registration) payload.state_registration = Number(form.value.state_registration)
    if (form.value.municipal_registration) payload.municipal_registration = Number(form.value.municipal_registration)
    if (form.value.tax_regime) payload.tax_regime = Number(form.value.tax_regime)
    if (form.value.street) payload.street = form.value.street
    if (form.value.address_number) payload.address_number = Number(form.value.address_number)
    ...
    if (form.value.zip_code) payload.zip_code = Number(String(form.value.zip_code).replace(/\D/g, ''))
    ...
    const detail = e?.data?.data?.error ?? e?.data?.message ?? e?.message ?? 'Erro desconhecido'
    toast.add({ title: 'Erro ao salvar empresa', description: detail, color: 'error' })
  } finally {
...
async function confirmDelete() {
  ...
  try {
    await $fetch(`/api/fiscal/company/${deletingCompany.value.id}`, { method: 'DELETE' })
    ...
    const detail = e?.data?.data?.error ?? e?.data?.message ?? e?.message ?? 'Erro desconhecido'
    toast.add({ title: 'Erro ao excluir empresa', description: detail, color: 'error' })
  } finally {
...
<template>
  ...
  <!-- ─── Create / Edit Modal ──────────────────────────────────────────────── -->
  <UModal v-model:open="showFormModal" :title="editingCompany ? 'Editar empresa' : 'Nova empresa'" size="xl">
    <template #body>
      <div class="space-y-6">
        <!-- Identification -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-3">Identificação</p>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <UFormField label="CNPJ">
              <UInput v-model="form.business_id" placeholder="00.000.000/0000-00" />
            </UFormField>
            <UFormField label="CPF (pessoa física)">
              <UInput v-model="form.individual_id" placeholder="000.000.000-00" />
            </UFormField>
            ...
            <UFormField label="Inscrição Municipal">
              <UInput v-model="form.municipal_registration" placeholder="Número" />
            </UFormField>
        ...
        <!-- Address -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-3">Endereço</p>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <!-- Contact -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-3">Contato</p>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <UFormField label="E-mail">
              <UInput v-model="form.email" type="email" placeholder="email@empresa.com" />
            </UFormField>
        ...
        <!-- Features -->
        <div>
          <p class="text-xs font-semibold text-muted uppercase tracking-wide mb-3">Configurações</p>

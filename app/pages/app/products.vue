  ...
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar title="Produtos">
        <template #right>
          <UButton v-if="canCreate" label="Novo produto" icon="i-lucide-plus" color="neutral" @click="openCreate" />
        </template>
      </UDashboardNavbar>
    </template>

    <div v-if="!canRead" class="p-6">
      <p class="text-sm text-muted">Você não tem permissão para visualizar produtos.</p>
    </div>
    ...
    <template v-else>
      <div class="flex flex-wrap gap-3 p-4 border-b border-default">
        ...
        <USelectMenu v-model="isServiceFilter" :items="typeFilterOptions" value-key="value" class="w-36" />
      </div>
      ...
      <div class="space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <UFormField label="Categoria">
            <USelectMenu v-model="form.category_id" :items="categoryOptions" value-key="value" class="w-full" placeholder="Sem categoria" searchable />
          </UFormField>

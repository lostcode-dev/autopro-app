...
<template>
  <UDashboardPanel>
    <template #header>
      <AppPageHeader title="Detalhes do funcionário">
        <template #right>
          <UButton
            label="Voltar"
            color="neutral"
    ...
    <template #body>
      ...
      <div v-else class="space-y-4 p-4">
        ...
        <template v-else>
          ...
          <div class="rounded-2xl border border-default bg-default/30 p-4">
            <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              <div class="grid gap-4 md:grid-cols-2 xl:w-md">
                <UFormField label="Data inicial">
                  <UiDatePicker v-model="dateFrom" class="w-full" />
                </UFormField>
                <UFormField label="Data final">
                  <UiDatePicker v-model="dateTo" class="w-full" />
                </UFormField>
              </div>

              <div class="flex flex-wrap items-center gap-2">
                <div class="rounded-xl border border-default/70 bg-default/50 px-3 py-2 text-sm text-muted">
                  Média por item: <span class="font-medium text-highlighted">{{ formatCurrency(summary.averageCommission) }}</span>
                </div>
                <UTooltip text="Exportar período">
                  <UDropdownMenu :items="exportItems" :content="{ align: 'end' }" :ui="{ content: 'min-w-44' }">
                    <UButton
                ...
                <UButton label="Atualizar" color="neutral" variant="ghost" icon="i-lucide-refresh-cw" @click="refresh" />
              </div>
            </div>
          </div>

          <div class="rounded-2xl border border-default bg-default/20 p-2">
            <UTabs :items="tabItems" :model-value="activeTab" @update:model-value="activeTab = $event as string" />
          </div>
          ...
          <AppDataTable
            ...
            :show-footer="false"
            show-search
            search-placeholder="Buscar por item, cliente, OS ou referência..."
            empty-icon="i-lucide-hand-coins"
            empty-title="Nenhuma comissão encontrada"
            empty-description="Não há comissões para o período selecionado."
          >
            <template #referenceDate-cell="{ row }">
              {{ formatDate(row.original.referenceDate) }}
            </template>
            ...
            <template #orderNumber-cell="{ row }">
              <div class="space-y-1">
                ...
                <UBadge
                  v-if="row.original.orderStatus"
                  :color="orderStatusColor(row.original.orderStatus)"
                  variant="subtle"
                  size="sm"
                >
            ...
            <template #itemName-cell="{ row }">
              <div class="space-y-1">
                <p class="font-medium text-highlighted">
                  {{ row.original.itemName || 'Item sem nome' }}
                </p>
                <p v-if="row.original.description" class="text-xs text-muted">
                  {{ row.original.description }}
                </p>
              </div>
            </template>

            <template #orderClientName-cell="{ row }">
              <span class="text-sm">{{ row.original.orderClientName || '—' }}</span>
            </template>

            <template #amount-cell="{ row }">
              <span class="font-semibold text-success">{{ formatCurrency(row.original.amount) }}</span>
            </template>

            <template #status_col-cell="{ row }">
              <UBadge :color="commissionStatusColor(row.original.status)" variant="subtle" size="sm">
                {{ commissionStatusLabel(row.original.status) }}
              </UBadge>
            </template>
            ...
            <template #actions-cell="{ row }">
              <div class="flex items-center justify-end gap-1">
                <UTooltip text="Ver cálculo da comissão">
                  <UButton
                    icon="i-lucide-eye"
                    color="neutral"
                    variant="ghost"
                    size="xs"
                    @click="openCommissionDetail(row.original)"
                  />
                </UTooltip>
              </div>
            </template>
          ...
          <AppDataTable
            ...
            v-model:search-term="searchTerm"
            :columns="itemColumns"
            :data="itemTableRows as unknown as Record<string, unknown>[]"
            :loading="status === 'pending'"
            :show-footer="false"
            show-search
            search-placeholder="Buscar por item, cliente ou OS..."
            empty-icon="i-lucide-package-search"
            empty-title="Nenhum item com comissão"
            empty-description="Os itens comissionados do período aparecerão aqui."
          >
            <template #itemName-cell="{ row }">
              <div class="space-y-1">
                <p class="font-medium text-highlighted">
                ...
                <p class="text-xs text-muted">
                  {{ row.original.orderClientName || 'Cliente não informado' }}
                </p>
              </div>
            </template>
            ...
            <template #orderNumber-cell="{ row }">
              <div class="space-y-1">
                ...
                <p class="text-xs text-muted">
                  Entrada {{ formatDate(row.original.orderEntryDate) }}
                </p>
              </div>
            </template>

            <template #baseAmount-cell="{ row }">
              <div class="space-y-1 text-right">
                <p class="font-medium text-highlighted">{{ formatCurrency(row.original.baseAmount) }}</p>
                <p class="text-xs text-muted">{{ row.original.commissionBase === 'profit' ? 'Base em lucro' : 'Base em receita' }}</p>
              </div>
            </template>

            <template #commissionRule-cell="{ row }">
              <span class="text-sm">{{ row.original.commissionRule }}</span>
            </template>

            <template #itemAmount-cell="{ row }">
              <span class="font-medium">{{ formatCurrency(row.original.itemAmount) }}</span>
            </template>

            <template #itemProfit-cell="{ row }">
              <span class="font-medium" :class="row.original.itemProfit >= 0 ? 'text-success' : 'text-error'">
                {{ formatCurrency(row.original.itemProfit) }}
              </span>
            </template>

            <template #amount-cell="{ row }">
              <div class="space-y-1 text-right">
                <p class="font-semibold text-success">{{ formatCurrency(row.original.amount) }}</p>
                <UBadge :color="commissionStatusColor(row.original.status)" variant="subtle" size="sm">
                  {{ commissionStatusLabel(row.original.status) }}
                </UBadge>
              </div>
            ...
            <template #item_actions-cell="{ row }">
              <div class="flex items-center justify-end gap-1">
                <UTooltip text="Ver cálculo da comissão">
                  <UButton

      ...
      <div v-else class="p-4 space-y-4">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <UPageCard variant="subtle" class="text-center">
            <p class="text-lg font-bold text-green-600">{{ formatCurrency(report.grossRevenue ?? report.totalRevenue ?? 0) }}</p>
            <p class="text-xs text-muted">Receita bruta</p>
          </UPageCard>
          <UPageCard variant="subtle" class="text-center">
            <p class="text-lg font-bold text-red-500">{{ formatCurrency(report.totalCosts ?? 0) }}</p>
            <p class="text-xs text-muted">Custos totais</p>
          </UPageCard>
          <UPageCard variant="subtle" class="text-center">
            <p class="text-lg font-bold text-blue-600">{{ formatCurrency(report.netProfit ?? 0) }}</p>
            <p class="text-xs text-muted">Lucro líquido</p>
          </UPageCard>
          <UPageCard variant="subtle" class="text-center">
            <p class="text-lg font-bold">{{ formatPercent(report.profitMargin ?? 0) }}</p>
            <p class="text-xs text-muted">Margem de lucro</p>
          </UPageCard>
        </div>
        ...
        <UPageCard v-if="report.costBreakdown" title="Detalhamento de Custos" variant="subtle">
          <dl class="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-2 text-sm">
            <div v-for="(value, key) in report.costBreakdown" :key="key">
              ...
              <dd class="font-medium">{{ formatCurrency(value) }}</dd>
            </div>
          </dl>
        </UPageCard>

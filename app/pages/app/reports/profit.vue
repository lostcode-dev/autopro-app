      ...
      <div v-else class="p-4 space-y-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <UPageCard variant="subtle" class="text-center">
            <p class="text-lg font-bold text-green-600">{{ formatCurrency(current.grossRevenue ?? 0) }}</p>
            <p class="text-xs text-muted">Receita bruta</p>
          </UPageCard>
          <UPageCard variant="subtle" class="text-center">
            <p class="text-lg font-bold text-red-500">{{ formatCurrency(current.totalCosts ?? 0) }}</p>
            <p class="text-xs text-muted">Custos</p>
          </UPageCard>
          <UPageCard variant="subtle" class="text-center">
            <p class="text-lg font-bold text-blue-600">{{ formatCurrency(current.netProfit ?? 0) }}</p>
            <p class="text-xs text-muted">Lucro líquido</p>
          </UPageCard>
          <UPageCard variant="subtle" class="text-center">
            <p class="text-lg font-bold">{{ formatPercent(current.profitMargin ?? 0) }}</p>
            <p class="text-xs text-muted">Margem</p>
          </UPageCard>
        </div>
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            <div v-for="(val, key) in variations" :key="key" class="flex items-center gap-2">
              ...
              <div>
                ...
                <p :class="['font-medium', variationColor(Number(val))]">{{ formatPercent(Number(val)) }}</p>
              </div>
            </div>
          </div>
        </UPageCard>

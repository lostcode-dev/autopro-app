  ...
  <UDashboardPanel>
    ...
    <template #body>
      <div class="grid grid-cols-3 divide-x divide-default border-b border-default text-center text-sm py-3">
        <div>
          <p class="text-muted text-xs">Total comprado</p>
          <p class="font-bold">{{ formatCurrency(summary.totalPurchased ?? 0) }}</p>
        </div>
        <div>
          <p class="text-muted text-xs">Total pago</p>
          <p class="font-bold text-green-600">{{ formatCurrency(summary.totalPaid ?? 0) }}</p>
        </div>
        <div>
          <p class="text-muted text-xs">Pendente</p>
          <p class="font-bold text-yellow-500">{{ formatCurrency(summary.totalPending ?? 0) }}</p>
        </div>
      </div>

...
<template>
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar title="Relatório de Fornecedores">
    ...
    <template #body>
      <div class="grid grid-cols-3 divide-x divide-default border-b border-default text-center text-sm py-3">
        <div>
          <p class="text-muted text-xs">Total em compras</p>
          <p class="font-bold">{{ formatCurrency(summary.totalPurchased ?? 0) }}</p>
        </div>
        <div>
          <p class="text-muted text-xs">Fornecedores ativos</p>
          <p class="font-bold">{{ summary.supplierCount ?? items.length }}</p>
        </div>
        <div>
          <p class="text-muted text-xs">Total de compras</p>
          <p class="font-bold">{{ summary.purchaseCount ?? 0 }}</p>
        </div>
      </div>

      <div class="p-4 border-b border-default">
        <UInput v-model="search" placeholder="Buscar fornecedor..." icon="i-lucide-search" class="w-72" @update:model-value="page = 1" />
      </div>

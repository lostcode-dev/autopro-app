  ...
  <UDashboardPanel>
    ...
    <template #body>
      <div class="grid grid-cols-3 divide-x divide-default border-b border-default text-center text-sm py-3">
        <div>
          <p class="text-muted text-xs">Total itens</p>
          <p class="font-bold">{{ summary.totalItems ?? summary.totalQuantity ?? 0 }}</p>
        </div>
        <div>
        ...
        <div>
          <p class="text-muted text-xs">Lucro total</p>
          <p class="font-bold text-blue-600">{{ formatCurrency(summary.totalProfit ?? 0) }}</p>
        </div>
      </div>

      <div class="p-4 border-b border-default">
        <UInput v-model="search" placeholder="Buscar item..." icon="i-lucide-search" class="w-72" @update:model-value="page = 1" />
      </div>

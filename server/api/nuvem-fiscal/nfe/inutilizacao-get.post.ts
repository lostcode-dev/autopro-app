...
export default defineEventHandler(async (event) => {
  ...
  const { response, responseBodyRaw } = await monitoredNuvemFiscalFetch({
    ...
    functionName: 'getNuvemFiscalNfeInutilizacao',
    url: `${apiBaseUrl}/nfe/inutilizacoes/${encodeURIComponent(id)}`,
    init: {
      method: 'GET',
      headers: {
  ...
  if (!response.ok) {
    throw createError({
      ...
      data: { error: 'Erro ao consultar inutilização de NF-e', details: data },
    })
  }

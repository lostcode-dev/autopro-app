...
export default defineEventHandler(async (event) => {
  ...
  const { response } = await monitoredNuvemFiscalFetch({
    ...
    functionName: 'downloadNuvemFiscalNfeEventoPdf',
    url: `${apiBaseUrl}/nfe/eventos/${encodeURIComponent(id)}/pdf`,
    init: {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    throw createError({
      ...
      data: { error: 'Erro ao baixar PDF do evento de NF-e', details: errText },
    })
  }

  const contentType = response.headers.get('content-type') || 'application/pdf'
  const fileName = extractFilename(response.headers.get('content-disposition'), `evento-nfe-${id}.pdf`)

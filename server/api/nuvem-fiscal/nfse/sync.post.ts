...
export default defineEventHandler(async (event) => {
  ...
  const { response, responseBodyRaw } = await monitoredNuvemFiscalFetch({
    ...
    functionName: 'syncNuvemFiscalNfse',
    url: `${apiBaseUrl}/nfse/${encodeURIComponent(id)}/sincronizar`,
    init: {
      method: 'POST',
      headers: {
  ...
  if (!response.ok) {
    throw createError({
      ...
      data: { error: 'Erro ao sincronizar NFS-e na Nuvem Fiscal', details: data },
    })
  }

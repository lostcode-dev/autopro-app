...
export default defineEventHandler(async (event) => {
  ...
  if (!body || typeof body !== 'object') {
    throw createError({ statusCode: 400, message: 'Payload da NFS-e é obrigatório' })
  }
  ...
  const { response, responseBodyRaw } = await monitoredNuvemFiscalFetch({
    ...
    functionName: 'createNuvemFiscalNfse',
    url: `${apiBaseUrl}/nfse`,
    init: {
      method: 'POST',
      headers: {
  ...
  if (!response.ok) {
    throw createError({
      ...
      data: { error: 'Erro ao emitir NFS-e na Nuvem Fiscal', details: data },
    })
  }

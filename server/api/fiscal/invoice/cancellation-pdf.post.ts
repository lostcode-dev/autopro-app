...
export default defineEventHandler(async (event) => {
  ...
  const { response } = await monitoredNuvemFiscalFetch({
    ...
    functionName: 'downloadNuvemFiscalNfeCancelamentoPdf',
    url: `${apiBaseUrl}/nfe/${encodeURIComponent(id)}/cancelamento/pdf`,
    init: {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    throw createError({
      ...
      data: { error: 'Erro ao baixar PDF de cancelamento da NF-e', details: errText }
    })
  }

  const contentType = response.headers.get('content-type') || 'application/pdf'
  const fileName = extractFilename(response.headers.get('content-disposition'), `cancelamento-nfe-${id}.pdf`)

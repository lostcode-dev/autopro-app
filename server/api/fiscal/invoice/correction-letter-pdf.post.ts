...
export default defineEventHandler(async (event) => {
  ...
  const { response } = await monitoredNuvemFiscalFetch({
    ...
    functionName: 'downloadNuvemFiscalNfeCartaCorrecaoPdf',
    url: `${apiBaseUrl}/nfe/${encodeURIComponent(id)}/carta-correcao/pdf`,
    init: {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    throw createError({
      ...
      data: { error: 'Erro ao baixar PDF da carta de correção da NF-e', details: errText }
    })
  }

  const contentType = response.headers.get('content-type') || 'application/pdf'
  const fileName = extractFilename(response.headers.get('content-disposition'), `carta-correcao-nfe-${id}.pdf`)

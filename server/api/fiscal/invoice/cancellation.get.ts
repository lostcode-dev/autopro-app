...
export default defineEventHandler(async (event) => {
  ...
  const { response, responseBodyRaw } = await monitoredNuvemFiscalFetch({
    ...
    functionName: 'getNuvemFiscalNfeCancelamento',
    url: `${apiBaseUrl}/nfe/${encodeURIComponent(id)}/cancelamento`,
    init: {
      method: 'GET',
      headers: {
  ...
  if (!response.ok) {
    throw createError({
      ...
      data: { error: 'Erro ao consultar cancelamento da NF-e', details: data }
    })
  }

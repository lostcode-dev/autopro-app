...
export default defineEventHandler(async (event) => {
  ...
  if (!body || typeof body !== 'object') {
    throw createError({ statusCode: 400, message: 'Payload de inutilização é obrigatório' })
  }
  ...
  const { response, responseBodyRaw } = await monitoredNuvemFiscalFetch({
    ...
    functionName: 'createNuvemFiscalNfeInutilizacao',
    url: `${apiBaseUrl}/nfe/inutilizacoes`,
    init: {
      method: 'POST',
      headers: {
  ...
  if (!response.ok) {
    throw createError({
      ...
      data: { error: 'Erro ao criar inutilização de NF-e na Nuvem Fiscal', details: data },
    })
  }
